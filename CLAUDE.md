...
- `lib/theme/` — `GameColorTheme` (an enum of seed colors run through `ColorScheme.fromSeed`, so every palette keeps WCAG AA contrast automatically) and `ThemeStore` (`shared_preferences`-backed save/load of the chosen palette). App-wide, not per-game, so Block puzzle will pick up the same palette choice once it exists. `GameApp` in `lib/main.dart` owns the current theme and hands a change callback down to each game screen; `SudokuScreen`'s palette icon opens a swatch picker built on this.
...
- `lib/features/sudoku/` — `SudokuScreen` wires the engine to UI: cell selection, a notes/candidate-number mode (toggled via the AppBar icon, distinguished by filled-vs-outline icon so it's not color-only), conflict indicators (red + underline), and a new-game bottom sheet for size/difficulty. Conflicts intentionally use red+underline rather than color alone plus an icon badge — an earlier icon badge was dropped because at cell scale it read as a stray digit rather than a warning mark; underline still keeps the signal non-color-only.
